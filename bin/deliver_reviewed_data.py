...
from egcg_core.app_logging import AppLogger, LoggingConfiguration
from egcg_core.exceptions import EGCGError

log_cfg = LoggingConfiguration(cfg)


class DataDeliveryError(EGCGError):
    pass
...
class DataDelivery(AppLogger):
    ...
    def get_deliverable_projects_samples(self, project_id=None, sample_id=None):
        ...
        for sample in samples:
            ...
            if not processes[-1].get('status','new') == 'finished':
                raise DataDeliveryError("Reviewed sample %s not marked as finished"%sample.get('sample_id'))
        ...
        for sample in self.all_samples_values:
            ...
            if sample.get('project_id') == project_id:
                ...
                if self.get_sample_species(sample.get('sample_id')) == 'Homo sapiens':
                    ...
                    if not tr:
                        raise DataDeliveryError('Sample %s has no total number of reads'%sample.get('sample_id'))
                    res.append(str(tr))
    ...
    def get_list_of_file_to_move(self, sample_name):
        ...
        if species is None:
            raise DataDeliveryError('No species information found in the LIMS for ' + sample_name)
    ...
    def deliver_data(self, project_id=None, sample_id=None):
        ...
        for project in project_to_samples:
            ...
            for sample in samples:
                ...
                if not os.path.isdir(origin_sample_dir):
                    raise DataDeliveryError('Directory for sample %s in project %s does not exist'%(sample, project))
                ...
                for file_to_move in list_of_file_to_move:
                    ...
                    if not os.path.isfile(origin_file):
                        raise DataDeliveryError('File %s for sample %s in project %s does not exist'%(file_to_move, sample, project))
        ...
        if self.dry_run:
            ...
            for command in self.all_commands:
                ...
                if exit_status != 0:
                    raise DataDeliveryError('command %s exited with status %s'%(command, exit_status))
